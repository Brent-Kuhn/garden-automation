...
function getDevice(req, res, next) {
  getSensors(req.params.deviceId)
  .then(srows=>getData(srows))
  .then(drows=>filterRows(drows))
  .then(filteredRows=>{
      let filterData = filteredRows;
      // console.log(filterData);
      res.render('device', { devices: filterData });
    }).catch(error => {console.log(error.message)});
};

function getSensors(id) {
  return new Promise ((resolve, reject) => {
    db.all(`SELECT deviceID, name FROM "alias" WHERE deviceID LIKE ?`, [id], function(err, alias_rows) {
      if(err) {
        reject(err);
      } else {
